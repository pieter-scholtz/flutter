...
#1      _rootRunUnary (dart:async/zone.dart:1141:38)''');
  ...
  Future<void> verifyCrashReportSent(
    ...
    expect(
        crashInfo.fields?['error_message'], 'Bad state: Test bad state error');
    ...
    expect(logger.traceText,
        contains('Crash report sent (report ID: test-report-id)'));
  }

  testWithoutContext('CrashReporter.informUser provides basic instructions',
      () async {
    ...
    expect(logger.errorText,
        contains('A crash report has been written to ${file.path}.'));
    expect(logger.statusText,
        contains('https://github.com/flutter/flutter/issues/new'));
  });
  ...
  group('allow analytics', () {
    ...
    testWithoutContext(
      ...
      expect(logger.errorText,
          contains('Failed to send crash report due to a network error'));
    });
    ...
    testWithoutContext('should send only one crash report when sent many times',
        () async {
      final RequestInfo requestInfo = RequestInfo();
    ...
    testWithoutContext('should not send a crash report if on a user-branch',
        () async {
      String? method;
      Uri? uri;
